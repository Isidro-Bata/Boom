/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package projecto_poo;

import java.io.FileInputStream;
import java.io.IOException;
import java.io.ObjectInputStream;
import java.util.Vector;

/**
 *
 * @author nazar
 */
public class verFuncio {
    private Funcionario cl;
    private Vector vt;
    private String nome;
    private float sal;
    private Pessoa ppl;
    
    public verFuncio(){
        vt=new Vector();
        
    }
    public boolean search(int cod) throws IOException{
        leitura("Funcionario");
        int verif=0;
        boolean res=false;
        int numEst = vt.size(); 
        for (int k=0;k<numEst; k++) { 
            ppl = (Pessoa)vt.elementAt(k); 
            if(ppl.getId()==cod & ppl instanceof Funcionario){
                verif=1;
                res=true;
            }
        }
        if(verif==0)
            System.out.println("Funcionario Nao Encontrado!");
        return res;
    }
    
    public boolean searchEmprestimo(int cod) throws IOException{
        leitura("Emprestimo");
        int verif=0;
        boolean res=false;
        int numEst = vt.size(); 
        Emprestimo temp; 
        for (int k=0;k<numEst; k++) { 
            temp = (Emprestimo)vt.elementAt(k); 
            if(temp.getIdEmpr()==cod){
                verif=1;
                res=true;
            }
        }
        if(verif==0)
            System.out.println("Nao Encontrado!");
        return res;
    }
   
    public void leitura(String nm){ //faz a leitura do ficheiro de obj e armazena em um array de obj
        try {
            FileInputStream ii = new FileInputStream (nm);
            ObjectInputStream tt = new ObjectInputStream (ii);
            vt = (Vector)tt.readObject();
            tt.close();
        } catch (ClassNotFoundException cc){
            System.out.println("Verifique a existencia da classe!!!"); 
        } catch (IOException pp){
            System.out.println("Problemas no leitura do ficheiro!");
        }

    }
    public float getSal(){return sal;}
    public String getNome(){return nome;}
}

